              ...
              <tbody>
                @foreach ($technology as $value)
                  <tr>
                    ...
                    <td>
                      <form action="{{ route('impact-tracker-tech.destroy', ['id' => $project->id, 'pivot_id' => $value->pivot->id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        @can('edit technology on project')
                          <a href="{{ route('impact-tracker-tech.edit',['id' => $project->id, 'pivot_id' => $value->pivot->id]) }}" class="btn btn-primary btn-xs">Edit</a>
                        @endcan
